      ...
      >
        <div className="aspect-[3/4] overflow-hidden relative bg-slate-950 flex items-center justify-center">
          {player.image ? (
            ...
            <div className="flex flex-col items-center justify-center text-slate-700">
              <Shield size={64} className="mb-2 opacity-30" />
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">No Photo</span>
            </div>
          )}
          ...
          <div className="absolute top-4 right-4 text-4xl font-black text-white/10 italic">
            {player.number ? `#${player.number}` : 'No. —'}
          </div>

          {/* Position Badge */}
          {player.position && (
            <div className="absolute bottom-4 left-4">
              <span className="bg-blue-600 text-white text-[10px] font-black px-2.5 py-1 rounded-sm uppercase tracking-wider">
                {player.position}
              </span>
            </div>
          )}
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <div className="bg-slate-950/50 p-2 rounded-lg border border-slate-800">
              <p className="text-[10px] text-slate-500 font-bold uppercase">Goals</p>
              <p className="text-sm font-black text-white">{player.stats?.goals || 0}</p>
            </div>
            <div className="bg-slate-950/50 p-2 rounded-lg border border-slate-800">
